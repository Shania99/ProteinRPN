...
# # Assuming GraphConv, FilterEdges, and other necessary components are defined elsewhere
...
#         in_channels: int,
#         ratio: Union[float, int] = 0.5,
#         GNN: torch.nn.Module = GraphConv,  # Make sure to provide a specific GNN class like GraphConv during initialization
#         min_score: Optional[float] = None,
#         multiplier: float = 1.0,
#         nonlinearity: Union[str, Callable] = "tanh",
#         **kwargs,
...
#         self.gnn = GNN(in_channels, 1, **kwargs)
#         # Use SelectTopK as is, no modifications required
#         self.select = SelectTopK(1, ratio, min_score, nonlinearity)
#         self.connect = (
#             FilterEdges()
#         )  # Assuming this is a method for edge filtering after pooling
...
#         # Directly compute scores from the GNN output, before any selection
#         scores = attn.squeeze()

#         # Continue with the original selection process
#         select_out = self.select(attn, batch)

#         perm = select_out.node_index
#         selected_scores = select_out.weight
#         assert selected_scores is not None

#         x = x[perm] * selected_scores.view(-1, 1)
#         x = self.multiplier * x if self.multiplier != 1 else x

#         connect_out = self.connect(select_out, edge_index, edge_attr, batch)

#         # Modifications: return the full score vector alongside the other outputs
#         return (
#             x,
#             connect_out.edge_index,
#             connect_out.edge_attr,
...
#             f"ratio={self.ratio}"
#             if self.min_score is None
#             else f"min_score={self.min_score}"
#         )
#         return f"{self.__class__.__name__}({self.gnn.__class__.__name__}, {self.in_channels}, {ratio}, multiplier={self.multiplier})"
...
# Assuming necessary GNN layer (e.g., GCNConv, GATConv) and other utilities are imported


class CustomNodeDropPoolingLayer(Module):
    def __init__(
        self,
        in_channels: int,
    ...
    def forward(
        ...
        # Adjust this section based on your specific pooling logic.
    ...
    def __repr__(self) -> str:
        return f"{self.__class__.__name__}({self.gnn.__class__.__name__}, {self.in_channels})"
