...
class GraphCNN(nn.Module):
    def __init__(self, channel_dims=[512, 512, 512], fc_dim=512, num_classes=256, pooling='MTP', pen_attn=False):
        ...
        if self.pooling == "MTP":
            if not pen_attn:
                ...
                self.pool = GraphMultisetTransformerPen(512, 256, 512, None, 10000, 0.25, ['GMPool_G', 'GMPool_G'], num_heads=8, layer_norm=True)
        else:
            self.pool = gmp
        # Define dropout
...
class CL_protNET(torch.nn.Module):
    def __init__(self, out_dim, esm_embed=True, pooling='MTP', pertub=False, pen_attn=False):
        ...
        if esm_embed:
            ...
            self.gcn = GraphCNN(pooling=pooling, pen_attn=pen_attn)
        else:
            self.gcn = GraphCNN(pooling=pooling, pen_attn=pen_attn)
...
##### Model is just GMT + readout, to get graph level features: transformer + pooling layer needed