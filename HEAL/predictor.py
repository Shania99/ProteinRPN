...
from my_utils import load_GO_annot
import esm
...
p.add_argument(
    ...
    default="bp",
    choices=["bp", "mf", "cc"],
    help="gene ontoly task",
)
p.add_argument("--pdb", type=str, default="", help="Input the query pdb file")
p.add_argument(
    "--esm1b_model", type=str, default="", help="The path of esm-1b model parameter."
)
p.add_argument(
    "--only_pdbch",
    default=False,
    type=str2bool,
    help="To use model parameters trained only on PDBch training set",
)
p.add_argument(
    "--prob",
    default=0.5,
    type=float,
    help="Output the function with predicted probility > 0.5 .",
)
args = p.parse_args()
...
if args.only_pdbch:
    model.load_state_dict(
        torch.load(f"model/model_{args.task}CL.pt", map_location=device)
    )
else:
    model.load_state_dict(
        torch.load(f"model/model_{args.task}CLaf.pt", map_location=device)
    )
model.eval()
with torch.no_grad():
