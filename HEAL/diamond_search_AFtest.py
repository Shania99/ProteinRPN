...
for idx, pdbch in enumerate(test_pdbch):
    ...
    print(idx, pdbch)
    os.system(
        "diamond blastp -d data/nrAF_PDB_train_sequences.dmnd --more-sensitive -q tmp.fa --outfmt 6 qseqid sseqid bitscore > diamond.res"
    )
...
y_true = np.stack(
    [
        np.concatenate(
            [
                prot2annot1[pdb_c]["mf"],
                prot2annot1[pdb_c]["bp"],
                prot2annot1[pdb_c]["cc"],
            ],
            -1,
        )
        for pdb_c in test_pdbch
    ]
)
