...
# go.obo
go_graph = obonet.read_obo(open("/om2/user/shania/datasets/HEAL_data/go-basic.obo", 'r'))
...
with open("/om2/user/shania/datasets/HEAL_data/ic_count.pkl",'rb') as f:
    ic_count = pkl.load(f)
...
class Metrics(object):
    def __init__(self, method_name, pckl_fn=None, opdict = {}):
        '''
        pick_fn is a pickle file with Y_true, Y_pred, goterms
        self.ont: ec, mf, bp, cc
        self.goterms: needed #need to see how to get (in predictor)
        self.gonames redundant
        self.proteins: redundant
        '''
        if pckl_fn is not None:
    ...
    def _cafa_go_aupr(self, labels, preds):
        ...
        for t in range(1, 10):
            ...
            recall = 0.0
            for i in range(0, n):
                # print(f"calculating prec, rec for each threhold {i}/{n-1}")
                pred_gos = set()
            ...
            print("calculating average prec, average recall")
            if m > 0:
                AvgPr = precision/m
                AvgRc = recall/n
    ...
    def _protein_centric_fmax(self, keep_pidx=None):
        ...
        if self.ont in {'mf', 'bp', 'cc'}:
            print("Inside protein centric fmax ont")
