...
@ck.option(
    "--model-file",
    "-mf",
    default="model/deepgoplusAF2_model.h5",
    help="DeepGOPlus model",
)
@ck.option(
    "--out-file",
    "-o",
    default="model/deepgoplusAF2_AF2test_predictions.pkl",
...
@ck.option(
    "--logger-file",
    "-lf",
    default="model/deepgoplusAF2_training.csv",
    help="Batch size",
)
@ck.option("--device", "-d", default="gpu:0", help="Prediction threshold")
@ck.option("--params-index", "-pi", default=-1, help="Definition mapping file")
def main(
    model_file,
    out_file,
    batch_size,
...
def create_model(nb_classes, params):
    ...
    for i in range(len(kernels)):
        ...
        pool = MaxPooling1D(pool_size=MAXLEN - kernels[i] + 1, name="pool_" + str(i))(
            conv
        )
...
class DFGenerator(Sequence):
    ...
    def __init__(self, set_type, batch_size):
        ...
        # else:
        prot2annot1, goterms1, gonames1, counts1 = load_GO_annot(
        ...
        self.graph_list = torch.load(
            os.path.join(self.processed_dir, f"{set_type}_graph.pt")
        )
        if set_type != "AF2test":
            self.graph_list = torch.load(
                os.path.join(self.processed_dir, f"{set_type}_graph.pt")
            ) + torch.load(os.path.join(self.processed_dir, f"AF2{set_type}_graph.pt"))
            self.pdbch_list = (
                torch.load(os.path.join(self.processed_dir, f"{set_type}_pdbch.pt"))[
                    f"{set_type}_pdbch"
                ]
                + torch.load(
                    os.path.join(self.processed_dir, f"AF2{set_type}_pdbch.pt")
                )[f"{set_type}_pdbch"]
            )
        else:
            self.pdbch_list = torch.load(
                os.path.join(self.processed_dir, f"{set_type}_pdbch.pt")
            )[f"test_pdbch"]
        ...
        for pdb_c in self.pdbch_list:
            if pdb_c in prot2annot.keys():
                self.y_true.append(
                    np.concatenate(
                        [prot2annot[pdb_c][task] for task in ["mf", "bp", "cc"]], -1
                    )
                )
            else:
                self.y_true.append(
                    np.concatenate(
                        [prot2annot1[pdb_c][task] for task in ["mf", "bp", "cc"]], -1
                    )
                )
    ...
    def __getitem__(self, idx):
        ...
        for idx in batch_index:
            ...
            data_onehot = np.concatenate(
                [
                    data_onehot,
                    np.zeros([self.MAXLEN - data_onehot.shape[0], 21]).astype(np.int64),
                ],
                0,
            )
        ...
        if self.start < self.size:
            ...
            for idx in batch_index:
                ...
                data_onehot = torch.nn.functional.one_hot(
                    graph.native_x.long(), 21
                ).numpy()
                data_onehot = np.concatenate(
                    [
                        data_onehot,
                        np.zeros([self.MAXLEN - data_onehot.shape[0], 21]).astype(
                            np.int64
                        ),
                    ],
                    0,
