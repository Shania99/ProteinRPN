...
class GoTermDataset(Dataset):
    ...
    def __init__(self, set_type, task, AF2model=False):
        ...
        self.y_true = np.stack(
            [prot2annot[pdb_c][self.task] for pdb_c in self.pdbch_list]
        )
        ...
        if AF2model:
            ...
            graph_list_af = torch.load(
                os.path.join(self.processed_dir, f"AF2{set_type}_graph.pt")
            )
            self.graph_list += graph_list_af
            self.pdbch_list_af = torch.load(
                os.path.join(self.processed_dir, f"AF2{set_type}_pdbch.pt")
            )[f"{set_type}_pdbch"]
            y_true_af = np.stack(
                [prot2annot[pdb_c][self.task] for pdb_c in self.pdbch_list_af]
            )
